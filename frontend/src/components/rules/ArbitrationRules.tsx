...
import { arbitrationRules } from '../../lib/arbitrationRules'
import { parseItalics, formatRules, groupRules } from "../../lib/formatters";

interface ArbitrationRulesProps {
  activeField: string;
}

export function ArbitrationRules({ activeField }: ArbitrationRulesProps) {
  // Map field names to rule keys
  const fieldToRuleKey: { [key: string]: keyof typeof arbitrationRules } = {
    'case_name': 'caseName',
    'award_description': 'awardDescription',
    'forum': 'forum',
    'case_award_number': 'caseAwardNumber',
    'pinpoint': 'pinpoint'
  };

  const ruleKey = fieldToRuleKey[activeField];
  if (!ruleKey || !arbitrationRules[ruleKey]) {
    return null;
  }

  const { title, rules, examples } = arbitrationRules[ruleKey];
