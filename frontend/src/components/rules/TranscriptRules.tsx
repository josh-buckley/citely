...
import { transcriptRules } from '../../lib/transcriptRules'
import { parseItalics, formatRules, groupRules } from "../../lib/formatters";
import { RulesTable } from "../ui/rules-table";

interface TranscriptRulesProps {
  activeField: string;
}

export function TranscriptRules({ activeField }: TranscriptRulesProps) {
  // Map field names to rule keys
  const fieldToRuleKey: { [key: string]: keyof typeof transcriptRules } = {
    'case_name': 'caseName',
    'court': 'court',
  ...
  if (!ruleKey || !transcriptRules[ruleKey]) {
    return null;
  }

  const { title, rules, examples, judicialOfficers } = transcriptRules[ruleKey];
